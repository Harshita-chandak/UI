...
import asyncio
import requests
server = "humor-retro-meeting-builders.trycloudflare.com "
...
async def queue_prompt_route(prompt_data: dict):
    ...
    try:
        ...
        logger.info(f"Full ComfyUI response: {response.status_code} - {response.text}")
...
async def object_remover(
    ...
    try:
        ...
        # Update JSON with uploaded file names
        prompt["2"]["inputs"]["categories"] = obj_name
        ...
        for attempt in range(max_retries):
            ...
            if history_response.status_code == 200:
                ...
                if history_data.get(f"{prompt_id}") and "outputs" in history_data[f"{prompt_id}"]:
                    try:
                        image_path = history_data[f"{prompt_id}"]["outputs"]["24"]["images"][0]["filename"]
                        print(f"Image is ready: {image_path}")
                        break
                    except (KeyError, IndexError):
                        pass  # Image is not yet available
